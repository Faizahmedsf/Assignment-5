...
const readFiledata = () => {
    return new Promise((resolve, reject) => {
        ...
        dbconfig_1.client.query('select users.*, customer.customer_name , customer.customer_role  from customer RIGHT JOIN users on customer.user_id = users.user_id order by users.user_id ASC', (err, result) => {
            if (err) {
                reject(err);
            }
            else {
